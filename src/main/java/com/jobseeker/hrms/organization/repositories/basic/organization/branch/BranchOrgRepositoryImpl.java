...
public class BranchOrgRepositoryImpl implements BranchOrgRepositoryExtend {
	...
	@Override
	public Optional<Branch> findFirstByActive(String branchId) {
		...
		return Optional.ofNullable(mongoTemplate.findOne(query, Branch.class));
	}

	@Override
	public Page<Branch> findByDataTables(Map<Object, Object> data) {
		PaginationParam param = RequestHandler.remapToData(data, PaginationParam.class);
		String companyId = ServletRequestAWS.getCompanyId();
		...
		List<Branch> resultList = mongoTemplate.find(queryBuilder, Branch.class);
		long total = mongoTemplate.count(Query.query(searchCriteria), Branch.class);
