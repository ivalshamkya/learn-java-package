...
public class HardSkillOrgOrgRepositoryImpl implements HardSkillOrgOrgRepositoryExtend {
    @Autowired
    @Qualifier("organizationMongoTemplate")
    private MongoTemplate mongoTemplate;
    
    @Override
    public Page<HardSkillOrganization> findByDataTables(PaginationParam param) {
        ...
        List<HardSkillOrganization> resultList = mongoTemplate.find(queryBuilder, HardSkillOrganization.class);
        long total = mongoTemplate.count(Query.query(searchCriteria), HardSkillOrganization.class);
        
        return new PageImpl<>(resultList, pageable, total);
    }
    
    @Override
    public Optional<HardSkillOrganization> findFirstByActive(String hardSkillId) {
        String companyId = ServletRequestAWS.getCompanyId();
        Query query = new Query(Criteria.where("_id").is(hardSkillId)
            .and("status").is(StatusData.ACTIVE)
            .and("company._id").is(companyId)
        );

        return Optional.ofNullable(mongoTemplate.findOne(query, HardSkillOrganization.class));
    }
}
