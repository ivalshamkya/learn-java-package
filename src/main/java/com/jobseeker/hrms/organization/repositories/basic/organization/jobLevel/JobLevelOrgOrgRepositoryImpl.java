...
public class JobLevelOrgOrgRepositoryImpl implements JobLevelOrgOrgRepositoryExtend {
	@Autowired
	@Qualifier("organizationMongoTemplate")
	private MongoTemplate mongoTemplate;

	@Override
	public Optional<JobLevel> findFirstByActive(String jobTypeId) {
		...
		return Optional.ofNullable(mongoTemplate.findOne(query, JobLevel.class));
	}

	@Override
	public Page<JobLevel> findByDataTables(PaginationParam param) {
		String companyId = ServletRequestAWS.getCompanyId();
		...
		List<JobLevel> resultList = mongoTemplate.find(queryBuilder, JobLevel.class);
		long total = mongoTemplate.count(Query.query(searchCriteria), JobLevel.class);
