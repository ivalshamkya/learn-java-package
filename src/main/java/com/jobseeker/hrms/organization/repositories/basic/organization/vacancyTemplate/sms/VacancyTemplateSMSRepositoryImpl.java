...
public class VacancyTemplateSMSRepositoryImpl implements VacancyTemplateSMSRepositoryExtend {
	@Autowired
	@Qualifier("organizationMongoTemplate")
	private MongoTemplate mongoTemplate;

	@Override
	public Optional<VacancyTemplateSMS> findFirstByActive(String branchSMSId) {
		...
		return Optional.ofNullable(mongoTemplate.findOne(query, VacancyTemplateSMS.class));
	}

	@Override
	public Page<VacancyTemplateSMS> findByDataTables(PaginationParam param) {
		String companyId = ServletRequestAWS.getCompanyId();
		...
		if(keyWord != null) {
			...
			Criteria regexName = Criteria.where("vacancyName").regex(escapedKeyword, "i");
		...
		List<VacancyTemplateSMS> resultList = mongoTemplate.find(queryBuilder, VacancyTemplateSMS.class);
		long total = mongoTemplate.count(Query.query(searchCriteria), VacancyTemplateSMS.class);
