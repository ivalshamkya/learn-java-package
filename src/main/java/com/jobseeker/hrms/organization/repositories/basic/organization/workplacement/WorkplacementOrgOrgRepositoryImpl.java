...
public class WorkplacementOrgOrgRepositoryImpl implements WorkplacementOrgOrgRepositoryExtend {
	@Autowired
	@Qualifier("organizationMongoTemplate")
	private MongoTemplate mongoTemplate;

	@Override
	public Optional<Workplacement> findFirstByActive(String jobTypeId) {
		...
		return Optional.ofNullable(mongoTemplate.findOne(query, Workplacement.class));
	}

	@Override
	public Page<Workplacement> findByDataTables(PaginationParam param) {
		String companyId = ServletRequestAWS.getCompanyId();
		...
		List<Workplacement> resultList = mongoTemplate.find(queryBuilder, Workplacement.class);
		long total = mongoTemplate.count(Query.query(searchCriteria), Workplacement.class);
