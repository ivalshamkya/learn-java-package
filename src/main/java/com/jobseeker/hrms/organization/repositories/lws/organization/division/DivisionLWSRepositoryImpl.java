...
public class DivisionLWSRepositoryImpl implements DivisionLWSRepositoryExtend {
	@Autowired
	@Qualifier("organizationMongoTemplate")
	private MongoTemplate mongoTemplate;

	@Override
	public Optional<DivisionLawson> findFirstByActive(String divisionId) {
		...
		return Optional.ofNullable(mongoTemplate.findOne(query, DivisionLawson.class));
	}

	@Override
	public Page<DivisionLawson> findByDataTables(PaginationParam param) {
		String companyId = ServletRequestAWS.getCompanyId();
		...
		List<DivisionLawson> resultList = mongoTemplate.find(queryBuilder, DivisionLawson.class);
		long total = mongoTemplate.count(Query.query(searchCriteria), DivisionLawson.class);
